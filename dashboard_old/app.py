...
# -----------------------------------------------------------------------------
    ...
    if cal is None or cal.empty:
        ...
        with left:
            if name:
                agg = cal.groupby(name, dropna=False)["_amount"].sum().sort_values(ascending=True).reset_index()
                agg["Amount"] = agg["_amount"] / 1e7
                fig = px.bar(agg, x="Amount", y=name, orientation="h", text="Amount")
                fig.update_layout(height=380, margin=dict(l=10,r=10,t=20,b=10), xaxis_title="₹ Crore")
                st.plotly_chart(fig, use_container_width=True)
        with right:
            if ctype:
                typ = cal.groupby(ctype)["_amount"].sum().sort_values(ascending=False).reset_index()
                typ["Amount"] = typ["_amount"] / 1e7
                fig = px.pie(typ, names=ctype, values="Amount", hole=.55)
